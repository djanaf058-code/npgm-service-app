# AI-чат с RAG по мануалам + эскалация на НПГМ-специалиста + i18n — дизайн

**Дата:** 19.05.2026
**Статус:** утверждено владельцем, готово к writing-plans
**Skill:** brainstorming → writing-plans (следующий шаг)

**Revision 2** (19.05.2026): добавлен раздел i18n как Phase 0. Английский UI —
требование пилотов KSA/UAE; делается до AI-чата, чтобы все новые экраны
сразу писались двуязычно.
...
## 3a. i18n — двуязычный UI (Phase 0)

### Цель

Пилотные компании (MCS — KSA, Gulf Explosives — UAE) работают
преимущественно на английском. Любой новый экран должен из коробки
существовать в RU и EN. Это **предварительное условие** AI-чата —
все UI-строки чата сразу будут двуязычными.

### Решения

| Аспект | Решение | Обоснование |
|---|---|---|
| Библиотека | **`next-intl` v3** | Стандарт для Next.js 15 App Router, server + client component, SEO-friendly |
| Хранилище переводов | `app/messages/{ru,en}.json` | JSON, namespace по фичам (`common.*`, `machines.*`, `tickets.*`, `ai.*`) |
| Текущий язык — где живёт | **`profiles.language`** (уже есть из миграции 0001) | Не плодим новых колонок |
| Fallback цепочка | `localStorage('npgm.locale')` → `profile.language` → `Accept-Language` → `'ru'` | Незалогиненный видит правильный язык по browser; залогиненный — свой выбор |
| URL-стратегия | **Без локали в URL** (cookie/localStorage-driven) | Меньше шума в роутах, проще; SEO не критичен для B2B SaaS |
| Picker | Header dropdown «RU / EN» с флагами, доступен на всех страницах (включая `/auth/login`) | Видно сразу при открытии |
| Что НЕ переводится | Пользовательские данные (имена машин, заметки, тексты тикетов) | Это контент клиента, должен оставаться как ввели |
| Email-уведомления | Берут `profiles.language` получателя | Чтобы НПГМ-специалист видел RU, оператор GE — EN |
...
Namespace-первая структура:

```json
{
  "common": { "save": "Save", "cancel": "Cancel", "loading": "Loading…" },
  "nav": { "machines": "Fleet", "tickets": "Tickets", "team": "Team" },
  "machines": { "title": "Machine card", "fleet": "Fleet of {{n}} machines" },
  "ai": { "ask_button": "Ask", "drawer_title": "Chat about {{model}}",
          "escalated_to_engineer": "Forwarded to engineer, ETA {{sla}}" },
  ...
}
```
...
1. **`next-intl` install + config** — `app/i18n.ts` с конфигом локалей, `middleware.ts` инжектит `NextIntlClientProvider`
2. **Извлечение строк** — все hardcoded русские строки переносятся в `ru.json`, EN-эквиваленты в `en.json`. Затрагивает ~30-50 строк в:
   - `AppLayout.tsx` (nav, role-badge)
   - `AdminLayout.tsx`
   - все pages в `/app/*` и `/admin/*` (~15 файлов)
   - все диалоги (Invite, DeleteCompany, MarkReceived, ...)
3. **Language picker** — компонент `<LocaleSwitcher>` в шапке layout-ов
4. **При login/onboarding** — сохраняем выбранный язык в `profiles.language`
5. **`useLocale()` / `useTranslations()`** в любом client component для текста

### Соответствие требованию владельца

- ✅ Выбор языка при открытии приложения — picker виден на login + во всех layout
- ✅ Английский → весь UI на английском (включая AI-чат, эскалации, email)
- ✅ Русский → всё на русском
- ✅ AI отвечает на выбранном языке — уже заложено в §6 (system-prompt берёт `profile.language`)
- ✅ Сохраняется между сессиями (`profiles.language` + cookie)

### Open question

После Phase 0 любой новый код **обязан** добавлять и RU и EN ключи —
иначе пилот UAE увидит «raw key» вместо текста. Это договорённость, не код-гарант.
В Phase 0 настроим `next-intl` так чтобы отсутствующий ключ в EN падал
в console.warn (не в crash) — облегчает отладку.
...
|---|---|---|---|
| **0** | i18n: `next-intl` setup + извлечение всех UI-строк в RU/EN + `<LocaleSwitcher>` + сохранение в `profiles.language` | 2-3 | весь существующий UI открывается и на RU и на EN; picker в шапке работает; новые экраны пишутся двуязычно по умолчанию |
...
| **B** | UI чата (drawer на машине) + БД для сообщений (без AI) — **сразу с RU/EN ключами** | 2-3 | оператор пишет, видит свою историю, фото грузятся в Storage |
| **C** | `/api/ai/respond` с RAG + Claude + streaming | 2-3 | AI отвечает на вопросы со ссылками на страницы РЭ, на языке `profile.language` |
...
**Параллелизация:** Phase 0 — обязательно первая. Дальше A и B можно делать параллельно. C зависит от A. D зависит от B+C. E зависит от D. F — последний.

**Минимально-полезный продукт:** 0 + A + B + C (~7-11 дней). Эскалация без AI можно отложить (оператор просто получает AI-ответ, без тикетов из чата).
...
- Дополнительные языки кроме RU и EN — v2 (текущий релиз только эти два)
